...
// 原始模具資料 (為了方便，直接在檔案中定義，實際專案中應從外部載入)
const rawData = [
...
const generateInitialParts = () => {
	...
	while (usedIndexes.size < rawData.length) {
		...
		if (blank[randGrid].length >= 3) continue;
...
const DraggablePart = ({ part, index, subIndex, children }) => {
	...
	return (
		...
		ref={drag}
		style={{
		opacity: isDragging ? 0.5 : 1, // 拖曳時半透明效果
		cursor: 'grab', // 拖曳時的滑鼠樣式
		width: '100%',
		display: 'flex',
		height: '100%'
		}}
		className={`grid-subcell hover-effect ${part ? part.type : 'empty'}`} // 樣式移到這裡
...
export default function MoldGridEnhanced() {
	// 從 localStorage 獲取已存儲的 parts，若無則生成新的
...
    // 每次 parts 更新時，將其保存到 localStorage
	useEffect(() => {
		localStorage.setItem('gridParts', JSON.stringify(parts));
	}, [parts]); // 註解：當 parts 變化時，將其保存到 localStorage，確保刷新後不變

	const [, drop] = useDrop(() => ({
		accept: 'PART', // 指定此區域可以接收的拖曳項目類型
		drop: (item, monitor) => {
		// 取得拖曳來源的索引和目標的索引
	...
	return (
		...
		{modalVisible && selectedPart && (
			<div className="modal-backdrop" style={{ backdropFilter: 'blur(3px)', backgroundColor: 'rgba(0, 0, 0, 0.2)', position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', zIndex: 1050, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
				<div style={{ width: '50%', height: '45%', backgroundColor: '#fff', borderRadius: '12px', border: '3px solid #989191', boxShadow: '0 4px 20px rgba(0,0,0,0.2)', fontFamily: 'Lenter', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
					<div style={{ backgroundColor: '#405E69', padding: '14px 16px', display: 'flex', justifyContent: 'flex-end' }}>
					<button type="button" className="btn-close btn-close-white" onClick={() => { setSelectedPart(null); setModalVisible(false); }}></button>
				</div>
				<div style={{ backgroundColor: '#F1F1F2', padding: '24px 30px', flex: 1, display: 'flex' }}>
					<div className="row g-4">
						...
						{ label: '在庫狀況', value: selectedPart.status },
							...
							<div className={index < 6 ? 'col-6' : 'col-12'} key={index} style={{ display: 'flex', alignItems: 'center', color: '#405E69', fontSize: '17px', fontWeight: 'bold' }}>
							<div style={{ width: '92px', height: '50px', backgroundColor: '#E7E1C4', color: '#405E69', fontSize: '16px', lineHeight: '50px', textAlign: 'center', borderRadius: '4px', marginRight: '24px', border: '2px solid #BDBDBD', fontWeight: 'bold' }}>{item.label}</div>
							{item.label === '在庫狀況' ? (
								<div style={{ display: 'flex', gap: '18px', flexWrap: 'wrap' }}>{item.value.split(' ').map((val, i) => <span key={i}>{val}</span>)}</div>
								) : (
					...
					<div style={{ width: '260px', display: 'flex', justifyContent: 'center', paddingLeft: '12px' }}>
						<img src={selectedPart.imageUrl} alt="模具示意圖" style={{ width: '220px', height: '220px', objectFit: 'contain', border: 'none' }} />
					</div>
				</div>
			</div>
		)}
